...
static void process_create(int connfd)
{
    if(fork() == 0){
        printf("Child created with id : %d\n",getpid());
        childFxn(connfd);  /* same function as before */
        printf("Child exited with id : %d\n",getpid());
        exit(0);
    }
}
